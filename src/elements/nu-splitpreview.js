...
export default class NuSplitPreview extends NuElement {
  ...
  ${this.nuContent.split('\n').length > 2 ? `<nu-btn
    ...
    <nu-code radius shadow padding="1x 2x" overflow="auto" grow="1" height="max 49.5x||max 35x|max 30x"><textarea>${this.nuContent}</textarea></nu-code>
  </nu-flow>
  <nu-block padding="2x left" border="1ow left inside #special-bg.50">${this.nuPreview}</nu-block>
</nu-grid>`;
    });
  }
